...
st.markdown(
    '<div class="banner">' + ''.join(f'<img src="{url}" />' for url in NBA_LOGOS) + '</div>',
    unsafe_allow_html=True
)
...
if page == "Dashboard":
    ...
    if odds_df.empty:
        ...
        filt = odds_df[odds_df["EV%"] >= cut]
        st.plotly_chart(px.histogram(filt, x="EV%", nbins=20, color_discrete_sequence=["#00ff88"]), use_container_width=True)
    ...
    if st.session_state.get("user_bets", []):
